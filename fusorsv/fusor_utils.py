...
def write_partitions_by_sample(sname_partition_path,P):
    for t in P:
        for b in P[t]:
            for c in P[t][b]:
                for s in P[t][b][c]:
                    path = sname_partition_path+'_S%s_T%s_B%s.pickle.gz'%(c,t,b)
                    S = {t:{b:{c:{s:P[t][b][c][s]}}}}
...
def read_partitions_by_caller(partition_path,callers,exclude,t,b,verbose=False):
    ...
    for c in set(callers).difference(set(exclude)):
        samples = glob.glob(partition_path+'*_S%s_T%s_B%s.pickle.gz'%(c,t,b)) #keys are in the pickler
        for sample in samples:
            with gzip.GzipFile(sample, 'rb') as f:
                start = time.time()
...
def read_partitions_by_sample(partition_path,sname):
    sample_data = glob.glob(partition_path+'/%s*.pickle.gz'%sname)
    Q = {}
    for i in range(len(sample_data)):
        with gzip.GzipFile(sample_data[i],'rb') as f: S = pickle.load(f)
        for t in S:
            if not Q.has_key(t):
                Q[t] = {}
            for b in S[t]:
...
def json_to_alpha(alpha_j):
    ...
    for t_j in alpha_j:
        ...
        for b_j in alpha_j[t_j]:
            b_i = int(b_j)
            alpha[t_i][b_i] = np.float64(alpha_j[t_j][b_j])
    return alpha
...
def pooled_distance(M,mode='j'):
    ...
    for t in M:
        ...
        for b in M[t]:
            ...
            for g in M[t][b]:
                D[t][b][g] = np.float64(1.0)  #default is the  max distance
                if mode=='j' and np.float64(M[t][b][g][1]) > 0.0:
                    #1-(|I|/|U|)
                    D[t][b][g] = np.float64(1.0)-np.float64(M[t][b][g][0])/np.float64(M[t][b][g][1])
                if mode=='u' and np.float64(M[t][b][g][1]) > 0.0 and np.float64(M[t][b][g][2]) > 0.0:
                    #1-2*((|I|/|U|)*(|D1|/(|D1|+|D2|))/((|I|/|U|)+(|D1|/(|D1|+|D2|))
                    j = np.float64(M[t][b][g][0])/np.float64(M[t][b][g][1])
                    u = np.float64(M[t][b][g][2])/(np.float64(M[t][b][g][2])+np.float64(M[t][b][g][3]))
                    D[t][b][g] = np.float64(1.0)-np.float64(2.0)*(j*u)/(j+u)
    ...
    for t in A:
        ...
        for b in A[t]:
            ...
            for g in A[t][b]:
                ...
                for i in range(4):
                    I[t][b][g][i] = np.float64(np.mean(A[t][b][g][np.where(A[t][b][g][:,0]!=k),i+1])+1)
    return I
...
def get_pwdm(D,G):
    ...
    for t in D:
        P[t] = {}
        for b in D[t]:
            P[t][b] = np.zeros((len(G),len(G)),dtype=np.float64)
            for i,j in it.combinations(range(len(G)),2):
                if D[t][b].has_key((G[i],G[j])):
                    ...
                    P[t][b][i][j] = P[t][b][j][i] = np.float64(1.0)
    return P
...
def sum_pairs(P):
    ...
    for t in P:
        ...
        for b in P[t]:
            S[t][b] = []
            for row in P[t][b]:
                S[t][b] += [np.sum(row)/np.float64(len(row)-1)]
    return S
...
def exp_stats(e,e_post,a):
    if a < np.float64(1.0):
        ...
        upper,lower,value = 0,0,np.float64(1.0)
        for row in H:
        ...
        x = [a,a,len(e),0,np.float64(0.0),np.float64(0.0),np.float64(0.0)]
    return x
...
def group_weight(g,k,D,NN,t,b):
    L,Q,w = [],[],np.float64(0.0)
    if NN[t][b].has_key(k):
        for i in NN[t][b][k]: #sort the group G
            if i[1] in g: Q += [i[1]]
        for i in range(len(Q)):
            c,j,n = np.float64(1.0),0,len(NN[t][b][Q[i]])
            while j<n and NN[t][b][Q[i]][j][1] == k: j+=1
            if i > 0: c = NN[t][b][Q[i]][j][0]        
            L += [np.float64(1.0)-(np.float64(1.0)-D[t][b][(k,Q[i])])*c]
        w = np.float64(1.0)-np.float64(np.prod(L))
    return w
...
def get_weight(A,E,average=0):
    if E.has_key((None,)): #default average weighting when no target is available
        if average>0:        #to do is to connect the independant expectations here
            c = np.float64(1.0)
            n = c/np.float64(average)
            w  = c-np.prod([c-n*np.float64((A[g][A[g].keys()[0]][1])) for g in A])
        else:
            w = np.float64(0.0) #0.0
    else:
...
def target_filter_cuttoff_static(B,static_filter):
    ...
    for t in B:
        alpha[t] = {}
        for b in B[t]:
            alpha[t][b] = np.float64(static_filter)
    return alpha
    ...
    for t in A:
        ...
        for b in A[t]:
            ...
            if len(J)>1:
                ...
                for k in J:
                    if J[k][1]>0.0: J[k] = float(np.float64(J[k][0])/np.float64(J[k][1]))
                    else:           J[k] = 0.0 
    ...
    for t in A:
        ...
        for b in A[t]:
            ...
            if len(J)>1:
                ...
                for p in J:
                    if J[p][1]>0.0: J[p] = float(np.float64(J[p][0])/np.float64(J[p][1]))
                    else:           J[p] = 0.0 
